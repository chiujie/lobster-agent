...
# 1. 環境變數 (請在 Render 後台設定這兩個)
OPENROUTER_KEY = os.getenv("OPENROUTER_API_KEY")
RESEND_API_KEY = os.getenv("RESEND_API_KEY")  # 你的 Resend API 金鑰
...
def send_lobster_email(to_email, subject, content):
    try:
        ...
        response = requests.post(
            ...
            json={
                "from": "LobsterBot <onboarding@resend.dev>", # 免費測試版發件人
                "to": [to_email],
                "subject": subject,
...
<body style="text-align:center; padding:50px; background:#0f172a; color:white; font-family:sans-serif;">
    ...
    <input id="msg" style="width:70%; padding:15px; border-radius:5px;" placeholder="例如：寄信給 1121410902@mail.hwu.edu.tw，主旨是你好，內容是龍蝦來了。" onkeypress="if(event.keyCode==13) send()"/>
    ...
    async function send() {
        ...
        try {
            ...
            document.getElementById('loading').innerText = "❌ 連線失敗，請檢查 Render Logs。";
        }
        box.scrollTop = box.scrollHeight;
    }
    </script>
